...
FORTUNE_DAILY_PRE_ANALYSIS_VERSION = 'v1.5.1'  # PR#56: annual 官殺 vocab fix (OFFICIAL_GODS in annual_enhanced.py) adds 官殺當令 to _compute_single_month's aspects.career for 偏官 day-pillars; aspects rides into the daily snapshot via the **monthly_result spread (vestigial — daily dimensions come from _dispatch_career), so bump for snapshot byte-correctness. v1.5.0: PR#55 followup 偏官 vocab fix (_dispatch_career _TG_GUANSHA + SPOUSE_STAR_FEMALE). v1.4.0: baseline Phase 2 (DR-3/DR-4/DR-5). v1.3.0: Components A–D.
FORTUNE_MONTHLY_PRE_ANALYSIS_VERSION = 'v1.2.1'  # PR#56: annual 官殺 vocab fix (OFFICIAL_GODS) adds 官殺當令 to aspects.career for 偏官 months -> MONTH dimensions.career score+signals shift (derived from aspects.career per monthly_enhanced). v1.2.0: PR#55 followup SPOUSE_STAR_FEMALE 偏官 romance fix. v1.1.0: Phase 2.x intraMonthBreakdown.
FORTUNE_YEARLY_PRE_ANALYSIS_VERSION = 'v1.2.1'  # PR#56: annual 官殺 vocab fix cascades into the 12-month career aggregation (YEAR 事業 dim) for 偏官 flow-months. v1.2.0: PR#55 followup SPOUSE_STAR_FEMALE 偏官 romance cascade (YEAR 感情 dim). v1.1.0: Phase 3 yearly_enhanced real output. Mirror FORTUNE_PRE_ANALYSIS_VERSIONS.year in prompts.ts.
