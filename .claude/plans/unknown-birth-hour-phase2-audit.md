...
### 2d — DONE + line-audited (2026-06-14) ✅ (live chat-message test env-blocked — see note)
Chat guard for hour-unknown. **Crash verdict (audit):** chat 422'd today at the engine DTO boundary (`ChatContextInput`/`FortuneChatContextInput` required `birth_time`) — the Python build fns were already 2a-safe. **Engine:** both chat DTOs → Optional `birth_time` + `hour_known` field; `/build-chat-context` + `/build-chat-context-fortune` thread `hour_known`; `build_chat_context` surfaces top-level `hourKnown`; `build_chat_context_fortune` threads `hour_known` into base + MONTH (`compute_single_month_by_yearmonth`+`compute_intra_month_breakdown`) + YEAR (`compute_year_by_year`) computes. **NestJS:** both fetch helpers send `hour_known` (profile loads full-row → hourKnown present); `ChatContext.hourKnown?`; `chat-prompt-builder.buildPrompt` prepends the 時辰未知 directive gated `chatContext.hourKnown === false` (before the 命盤資料 JSON). COMPATIBILITY deliberately untouched (profiles stay hour-known). Line audit CONFIRMED-CORRECT (both data-flow directions; 422 fixed; gate strict; cache-isolated; tsc 0). Regression locks: engine `test_unknown_hour.py` +4 (build_chat_context hourKnown + build_chat_context_fortune × DAY/MONTH/YEAR no-crash); NestJS `chat-prompt-builder.hour-unknown.spec.ts` (3 gate tests). Engine 22 + chat 97 + 3 gate tests green.
> **Live chat-message browser test NOT run (env-blocked):** CAREER chat is disabled in this dev env (`CHAT_ENABLED_READING_TYPES` unset → CAREER off); LIFETIME chat requires an unlocked reading (3 credits + full generation). The suppression DIRECTIVE + injection mechanism are byte-identical to the reading-page injectors (2c) + FORTUNE injectors, both of which were live-browser-verified to suppress fabrication. Static + unit + mechanism verification is strong; the chat-surface live message is the one belt-and-suspenders check deferred. (Bonus this session: fixed the pre-existing `/reading/fortune` 500 — `html2canvas` was declared but not installed; `npm install --ignore-scripts` restored it, page now 200.)

### 2d (orig placeholder) — Chat guard
