 ...
 *
 * RN port of apps/web/app/components/chat/hooks/useChatSession.ts. The only
 * changes from web are: Clerk import (@clerk/clerk-expo), getToken excluded
 * from dep arrays (fresh ref every render on Expo → infinite-loop guard), and
 * the tier-upgrade-refund dismissal persisted via AsyncStorage instead of
 * localStorage. Behavior + exported interface are otherwise identical.
 */
...
export interface UseChatSessionReturn {
  ...
  /** Dismiss the tier-upgrade refund banner. Persists to AsyncStorage so
   *  the dismissal survives app restarts. */
...
export function useChatSession(args: UseChatSessionArgs): UseChatSessionReturn {
  ...
  // RN: the explicit callable-type annotation forces the ternary to resolve
  // to a single `(args) => Promise<ChatSession[]>` type instead of a union of
  // the four branch types. That keeps the no-subject fallback a plain paramless
  // `() => Promise.resolve([])` callable at the `{ token }` call sites (the web
  // named an ignored `_t` param to keep the union uniform, which trips
  // @typescript-eslint/no-unused-vars under the mobile config).
  const listSessionsForCurrentSubject: (args: {
    token: string;
  }) => Promise<ChatSession[]> = fortune
    ? ({ token }: { token: string }) =>
        listSessionsForFortune({
  ...
  // (also persisted to AsyncStorage so dismissal survives app restarts).
  ...
  const requireToken = useCallback(async () => {
    ...
    return token;
    // RN: Clerk's getToken is a fresh reference on every render (Expo). Keeping
    // deps [] avoids re-creating requireToken each render, which would cascade
    // into infinite re-fetch through the downstream effects/callbacks that
    // depend on it. getToken is behaviorally stable even though its ref changes.
  ...
  const resumeOpenSession = useCallback(
    ...
    // `applyTierUpgradeRefundFromUsage` (a stable useCallback([]) declared
    // below) is the only other referenced value; it's forward-declared so it
    // cannot be listed here. Deps mirror web exactly.
    // eslint-disable-next-line react-hooks/exhaustive-deps
   ...
   * timestamp in AsyncStorage). RN: the dismissed-key read is async, so we
   * resolve it before deciding whether to show the banner — this preserves
   * the web's "never flash a dismissed banner" behavior.
   */
  const applyTierUpgradeRefundFromUsage = useCallback(
    (refund: ChatUsageResponse['recentTierUpgradeRefund']) => {
      ...
      AsyncStorage.getItem(dismissedKey)
        .then((value) => {
          setTierUpgradeRefund(value === '1' ? null : refund);
        })
        .catch(() => {
          // AsyncStorage unavailable — show banner anyway, user can dismiss.
  ...
  const dismissTierUpgradeRefund = useCallback(() => {
    ...
    // Fire-and-forget persist — the in-memory dismissal (setState below) is
    // authoritative for this session; the write just makes it survive an app
    // restart. Ignore storage failures (in-memory dismissal still works).
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps -- subject helpers are new refs each render; the primitive readingId/comparisonId/fortune?.* deps are the real triggers, and applyTierUpgradeRefundFromUsage is forward-declared (mirrors web)
  }, [
    readingId,
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps -- createSessionForCurrentSubject is a new ref each render; the primitive readingId/comparisonId/fortune?.* deps are the real triggers (mirrors web)
  }, [
    readingId,
    comparisonId,
    fortune?.profileId,
  ...
  const switchActiveSession = useCallback(
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- listSessionsForCurrentSubject is a new ref each render; the primitive readingId/comparisonId/fortune?.* deps are the real triggers (mirrors web)
    [
      readingId,
      comparisonId,
      fortune?.profileId,
  ...
  useEffect(() => {
    ...
    prevFortuneAnchorRef.current = fortune.fortuneAnchorDate;
    prevFortuneProfileRef.current = fortune.profileId;
  // eslint-disable-next-line react-hooks/exhaustive-deps -- whole `fortune` object omitted; the primitive fortune?.fortuneAnchorDate/profileId are the real change triggers (mirrors web)
