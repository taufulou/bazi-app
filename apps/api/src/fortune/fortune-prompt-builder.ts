...
function buildFortuneHourUnknownBlock(readingLabel: string, extraLines: string[] = []): string {
  return [
    ...
    '禁止「編造」或「詳細分析」時柱相關項目：時柱十神／藏干／神煞、子女相關（子女宮／子女緣分／子女數目／子女性格，並涵蓋「添丁、懷孕、生育、家庭添丁喜事」等任何子女或生育預測——即使在流日、流月、流年、家庭等段落，也一律不可預言生育或添丁）、晚年、命宮、身宮。',
    ...
    // 不加 geJuStatus/從格 條款：FORTUNE_V1_PROMPTS（日/月/年運模板）從不論格局/從格，且本函式無 dayMaster 取用權，加了亦為死碼（與 ai.service 的差異為刻意）。
    ...extraLines,
    '',
  ].filter(Boolean).join('\n');
}
